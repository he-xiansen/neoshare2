...
|------|----------|----------|
| 访客 | 无需注册 | 浏览和下载公共文件 |
| 普通用户 | 管理员创建 | 管理个人文件、上传下载、预览编辑、创建文件夹 |
...
| 主界面 | 文件资源管理器 | 显示文件/文件夹列表，支持拖拽上传（含文件夹）、新建文件夹、复制粘贴、格式切换，支持 URL 路径同步（如 /folder/subfolder） |
...
### 访客流程
访客访问网站 → 默认显示公共文件目录 → 浏览文件列表（支持进入子目录） → 点击下载文件 → 双击预览支持的文件格式

### 用户流程
用户登录 → 默认显示个人文件目录 → 可以切换查看公共目录 → 拖拽上传文件或文件夹 → 新建文件夹 → 双击编辑支持的文件格式 → 下载文件
...
| 主界面 | 文件资源管理器 | 顶部工具栏（新建文件夹、上传按钮、搜索框、视图切换按钮），主区域支持列表/图标视图，支持面包屑导航，文件项悬停显示操作按钮 |
